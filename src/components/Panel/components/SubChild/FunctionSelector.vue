<script lang="ts" setup>
  ...
  const getFunctionCategory = () => {
    axios
      .get('/workflow/rest/function-category', {
        params: {
          queryOptions: '{}'
        }
      })
      .then((response) => {
        if (response.data.status === 1) {
          ...
          getFunctions(response.data?.content[0]?.id)
        }
      })
  }

  const getFunctions = (categoryId: string) => {
    const queryOptions = {
      conditionGroup: {
  ...
  const nodeProps = ({ option }: { option: TreeOption }) => {
    return {
      onClick() {
        ...
        getFunctions(option.id as string)
      }
    }
  }
